...
static void fsl_imx6_init(Object *obj)
{
    ...
    for (i = 0; i < smp_cpus; i++) {
        ...
        object_property_add_child(obj, name, OBJECT(&s->cpu[i]), &error_abort);
    }
    ...
    object_property_add_child(obj, "ccm", OBJECT(&s->ccm), &error_abort);

    object_initialize(&s->src, sizeof(s->src), TYPE_IMX6_SRC);
    qdev_set_parent_bus(DEVICE(&s->src), sysbus_get_default());
    object_property_add_child(obj, "src", OBJECT(&s->src), &error_abort);
    ...
    object_property_add_child(obj, "gpt", OBJECT(&s->gpt), &error_abort);
    ...
    for (i = 0; i < FSL_IMX6_NUM_I2CS; i++) {
        ...
        object_property_add_child(obj, name, OBJECT(&s->i2c[i]), &error_abort);
    }
    ...
    for (i = 0; i < FSL_IMX6_NUM_ECSPIS; i++) {
        ...
        object_property_add_child(obj, name, OBJECT(&s->spi[i]), &error_abort);
    }

    object_initialize(&s->eth, sizeof(s->eth), TYPE_IMX_ENET);
    qdev_set_parent_bus(DEVICE(&s->eth), sysbus_get_default());
    object_property_add_child(obj, "eth", OBJECT(&s->eth), &error_abort);
}
