...
class TypeDeclarationFixup(FileMatch):
    ...
    def gen_patches(self) -> Iterable[Patch]:
        ...
        for uppercase,checkers in checker_dict.items():
            ...
            fvalues = dict((field, set(m.group(field) for m in checkers
                                        if field in m.match.groupdict()))
                            for field in fields)
    ...
    def find_conflicts(self, uppercase: str) -> Iterable[FileMatch]:
    ...
    def gen_patches_for_type(self, uppercase: str,
                             checkers: List[TypeDeclaration]) -> Iterable[Patch]:
        return
        yield
...
class DeclareVoidTypes(TypeDeclarationFixup):
    ...
    def gen_patches_for_type(self, uppercase: str,
                             checkers: List[TypeDeclaration]) -> Iterable[Patch]:
        conflicting = self.find_conflicts(uppercase)
        if conflicting:
            for c in checkers:
...
class AddDeclareTypeName(TypeDeclarationFixup):
    ...
    def gen_patches_for_type(self, uppercase: str,
                             checkers: List[TypeDeclaration]) -> Iterable[Patch]:
        typenames = set(c.typename for c in checkers
                        if c.typename is not None)
        if len(typenames) > 1:
            self.warn("ambiguous type names")
            for c in checkers:
                c.warn("typename is %s here", c.typename)

        if len(typenames) == 0:
            self.warn("can't find type name")
            return
...
def find_type_checkers(files: FileList, uppercase: str) -> Iterable[TypeCheckerDeclaration]:
    ...
    for c in (DeclareInstanceChecker, DeclareClassCheckers, DeclareObjCheckers, ObjectDeclareType, ObjectDeclareSimpleType):
        yield from files.find_matches(c, name=uppercase, group='uppercase')
