...
static Property cpu_x86_properties[] = {
    ...
    DEFINE_PROP_BIT("f-kvm-steal-time", X86CPU, env.cpuid_kvm_features,  5, true),
    DEFINE_PROP_BIT("f-kvm_pv_eoi", X86CPU, env.cpuid_kvm_features,  6, true),
    DEFINE_PROP_BIT("f-kvm-clock-stable", X86CPU, env.cpuid_kvm_features,  24, true),
            ...
            if (val) {
                *val = 0; val++;
                if (!strcmp(featurestr, "vendor")) {
                    ...
                    qdict_put(*features, "model-id", qstring_from_str(val));
                } else {
                    qdict_put(*features, featurestr, qstring_from_str(val));
                }
            } else {
...
void x86_cpu_realize(Object *obj, Error **errp)
{
    ...
    if (kvm_enabled() && check_cpuid && kvm_check_features_against_host(cpu)
        && enforce_cpuid) {
