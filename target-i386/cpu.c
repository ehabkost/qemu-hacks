...
#define DEFINE_PROP_HV_VAPIC(_n)                                               \
    DEFINE_GENERIC_PROP(_n, qdev_prop_hv_vapic)
...
#define DEFINE_PROP_CHECK(_n) \
    DEFINE_GENERIC_PROP(_n, qdev_prop_check)
...
#define DEFINE_PROP_ENFORCE(_n)                                                \
    DEFINE_GENERIC_PROP(_n, qdev_prop_enforce)
...
#define DEFINE_PROP_MODEL_ID(_n)                                               \
    DEFINE_GENERIC_PROP(_n, qdev_prop_model_id)
...
void x86_cpu_list(FILE *f, fprintf_function cpu_fprintf)
{
    ...
    QDEV_CLASS_FOREACH(dc, DEVICE_CLASS(object_class_by_name(TYPE_X86_CPU))) {
        QDEV_PROP_FOREACH(prop, dc) {
            if (strncmp(prop->name, "f-", 2) == 0) {
...
int cpu_x86_register(X86CPU *cpu, const char *cpu_model)
{
    ...
    compat_normalize_cpu_model(cpu_model, &cpu_name, &features, &error);
    if (error) {
        goto out;
    }
    ...
    if (cpu_x86_find_by_name(def, cpu_name) < 0) {
...
void x86_cpu_realize(Object *obj, Error **errp)
{
    ...
    if (check_cpuid && kvm_check_features_against_host(cpu)
        && enforce_cpuid) {
        error_setg(errp, "host's CPU doesn't have requested features");
        return;
    }
