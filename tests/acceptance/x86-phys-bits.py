...
class HostPhysbits(Test):
    ...
    def test_no_host_phys_bits(self):
        ...
        self.vm.launch()
        self.assertEquals(self.cpu_phys_bits(), 40)

    def test_manual_phys_bits(self):
        self.vm.add_args('-cpu', 'qemu64,host-phys-bits=off,phys-bits=35')
        self.vm.launch()
        self.assertEquals(self.cpu_phys_bits(), 35)

    def test_host_phys_bits(self):
        host_phys_bits = self.host_phys_bits()
        self.vm.add_args('-cpu', 'qemu64,host-phys-bits=on')
        self.vm.launch()
        self.assertEquals(self.cpu_phys_bits(), host_phys_bits)

    def test_host_phys_bits_limit_high(self):
        host_phys_bits = self.host_phys_bits()
        self.vm.add_args('-cpu', 'qemu64,host-phys-bits=on,host-phys-bits-limit=%d' % (host_phys_bits + 1))
        self.vm.launch()
        self.assertEquals(self.cpu_phys_bits(), host_phys_bits)

    def test_host_phys_bits_limit_equal(self):
        host_phys_bits = self.host_phys_bits()
        self.vm.add_args('-cpu', 'qemu64,host-phys-bits=on,host-phys-bits-limit=%d' % (host_phys_bits))
        self.vm.launch()
        self.assertEquals(self.cpu_phys_bits(), host_phys_bits)
    ...
    def test_host_phys_bits_limit_low(self):
        ...
        self.vm.add_args('-cpu', 'qemu64,host-phys-bits=on,host-phys-bits-limit=%d' % (host_phys_bits - 1))
        self.vm.launch()
        self.assertEquals(self.cpu_phys_bits(), host_phys_bits - 1)
