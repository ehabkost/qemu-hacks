...
static inline void usage_complete(int argc, char *argv[])
{
...
static void htable_init(bool pr)
{
...
/* clear init variables so that we can call qht_bench() several times */
static void clear_init_vars(void)
{
    test_start = false;
    test_stop = false;
    n_ready_threads = 0;
