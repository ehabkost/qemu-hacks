...
static void assert_valuelist_extend(const char *before,
                                    const char *extend,
                                    const char *after)
{
...
static void test_valuelist_extend(void)
{
    ...
    assert_valuelist_extend("[[1, 4], [6, 30], [40, 50], [53, 60]]",
                            "32",
                            "[[1, 4], [6, 30], 32, [40, 50], [53, 60]]");
    assert_valuelist_extend("[[1, 4], [6, 30], [40, 50], [53, 60]]",
                            "[[32, 35]]",
                            "[[1, 4], [6, 30], [32, 35], [40, 50], [53, 60]]");
    ...
    assert_valuelist_extend("[[1, 4], [6, 30], [40, 50], [53, 60]]",
                            "38",
                            "[[1, 4], [6, 30], 38, [40, 50], [53, 60]]");
    assert_valuelist_extend("[[1, 4], [6, 30], [40, 50], [53, 60]]",
                            "[[35, 38]]",
                            "[[1, 4], [6, 30], [35, 38], [40, 50], [53, 60]]");
...
static void test_slots_can_combine(void)
{
    ...
    slot_add_prop(a, "bus", JS("\"mybus.0\""));
    g_assert_false(slots_can_be_combined(a, b, &opt_name));
    slot_add_prop(b, "bus", JS("\"mybus.0\""));

    g_assert_true(slots_can_be_combined(a, b, &opt_name));
    g_assert_null(opt_name);

    slot_add_prop(a, "addr", JS("[ 1, 3 ]"));
    g_assert_false(slots_can_be_combined(a, b, &opt_name));
    slot_add_prop(b, "addr", JS("5"));

    g_assert_true(slots_can_be_combined(a, b, &opt_name));
    g_assert_cmpstr(opt_name, ==, "addr");

    slot_add_prop(a, "unit", JS("1"));
    g_assert_false(slots_can_be_combined(a, b, &opt_name));
    slot_add_prop(b, "unit", JS("1"));
    ...
    slot_add_prop(a, "port", JS("10"));
    g_assert_false(slots_can_be_combined(a, b, &opt_name));
    slot_add_prop(b, "port", JS("20"));
